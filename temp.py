...
TEMP_THRESHOLD = 60  # Default temperature threshold in Celsius
...
def send_data_to_server(timestamp, temperature, max_temperature, threshold_exceeded):
    if check_wifi():
        url = 'http://<your_pi_ip>/temperature_receiver.php'  # Replace <your_pi_ip> with your Raspberry Pi's IP address
        data = {
            'timestamp': timestamp,
...
# BlueRed Mode: Existing Mode
def run_blue_red_mode():
...
# Multiple Temp Mode: Dynamic Target Detection and Surrounding Temps
def run_multiple_temp_mode():
    ...
    while True:
        try:
            ...
            # Find the coordinates of the maximum temperature (assumed target)
            max_pos = np.unravel_index(np.argmax(data_array), data_array.shape)
            target_i, target_j = max_pos

            # Define the region around the target (e.g., 3x3 grid)
            region_size = 1  # Number of pixels around the target
            i_min = max(target_i - region_size, 0)
            i_max = min(target_i + region_size + 1, data_array.shape[0])
            j_min = max(target_j - region_size, 0)
            j_max = min(target_j + region_size + 1, data_array.shape[1])

            # Extract the region
            region = data_array[i_min:i_max, j_min:j_max]

            # Clear previous texts
            ax.texts = [text for text in ax.texts if text.get_text() == '']

            # Display temperatures for the region
            for i in range(region.shape[0]):
                for j in range(region.shape[1]):
                    ...
                    if temp > TEMP_THRESHOLD:
                        color = 'red'
                    elif temp > TEMP_THRESHOLD - 5:  # Adjust the threshold difference as needed
                        color = 'yellow'
                    else:
                        color = 'white'
                    ax.text(j_min + j, i_min + i, f'{temp:.1f}°C', color=color, fontsize=8, ha='center', va='center')
...
# HeatMap Mask Mode: Highlight areas above threshold
def run_heatmap_mask_mode():
    ...
    while True:
        try:
            ...
            masked_frame = cv2.addWeighted(np.fliplr(data_array), 0.7, mask, 0.3, 0)
...
# Menu for selecting view modes
def view_mode_menu():
    root = tk.Tk()
    root.title("View Mode Selection")
    ...
    # Create a label
    label = tk.Label(root, text="Select View Mode", font=("Helvetica", 16))
    label.pack(pady=20)

    # Radio buttons for modes
    tk.Button(root, text="BlueRed Mode", command=select_blue_red_mode, font=("Helvetica", 14), width=20).pack(pady=10)
    tk.Button(root, text="Multiple Temp Mode", command=select_multiple_temp_mode, font=("Helvetica", 14), width=20).pack(pady=10)
    tk.Button(root, text="HeatMap Mask Mode", command=select_heatmap_mask_mode, font=("Helvetica", 14), width=20).pack(pady=10)

    root.mainloop()

# Mode selection menu (Main Menu)
def mode_menu():
    root = tk.Tk()
